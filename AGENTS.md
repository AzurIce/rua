...
- `crates/rua-graph` — 图模型定义 + 图数据能力（纯库，不依赖 rig/axum/tokio，依赖仅 serde/serde_json/ulid/thiserror；不含 LLM/工具消费）：不可变节点（Input/Turn/Context；Input 携带展开后的工具覆盖列表、Turn 记录该轮有效工具集，旧数据缺字段读成空数组 = 未记录）、cursor 注册表、journal 事件溯源、`Graph::load_chain`（链回溯 + 材料解析）
